...
aiChatRouter.get('/health', async (req, res) => {
  try {
    ...
    if (key && key !== 'your_copied_api_key_here' && key !== '') {
      res.json({ success: true, gemini: true, ollama: true });
    } else {
      res.json({ success: true, gemini: false, ollama: false });
    }
  } catch (err) {
    res.json({ success: true, gemini: false, ollama: false });
  }
...
aiChatRouter.post('/sessions/:sessionId/messages', async (req, res) => {
  try {
    ...
    if (geminiApiKey && geminiApiKey !== 'your_copied_api_key_here' && geminiApiKey !== '') {
      try {
        ...
        // Fallback to non-history simple generation using 'gemini-2.5-flash' explicitly
        try {
          const genAI = new GoogleGenerativeAI(geminiApiKey);
          ...
          aiResponse = "I'm having trouble connecting to Gemini. Please verify your API key or try again later.";
        }
      }
    } else {
      aiResponse = "Gemini API key is missing. Please set GEMINI_API_KEY in your .env file.";
    }

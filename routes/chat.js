...
chatRouter.post('/requests', async (req, res) => {
  try {
    ...
    const tutorExists = tutors.some((t) => t.id === tutorId || t.userId === tutorId);
    if (!tutorExists) {
      // Also check users collection just in case they are approved but haven't updated profile yet
      const users = await store.users.get();
      const userTutor = users.find((u) => u.id === tutorId && ['tutor', 'tutor_pending'].includes((u.role || '').toLowerCase()));
      if (!userTutor) {

...
bookingsRouter.get('/tutor', authMiddleware, async (req, res) => {
  try {
    const role = (req.user?.role || '').toLowerCase();
    if (!role.includes('tutor')) return res.status(403).json({ error: 'Not authorized' });
...
bookingsRouter.patch('/:id/accept', authMiddleware, async (req, res) => {
  try {
    const { id } = req.params;
    const role = (req.user?.role || '').toLowerCase();
    if (!role.includes('tutor')) return res.status(403).json({ error: 'Only tutors can accept booking requests' });
...
bookingsRouter.patch('/:id/reject', authMiddleware, async (req, res) => {
  try {
    const { id } = req.params;
    const role = (req.user?.role || '').toLowerCase();
    if (!role.includes('tutor')) return res.status(403).json({ error: 'Only tutors can reject booking requests' });

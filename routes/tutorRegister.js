...
tutorRegisterRouter.get('/search', async (req, res) => {
  try {
    ...
    const withRatings = tutors.map((t) => {
      ...
      const instObj = institutes.find((i) => i.id === t.instituteId);
      return {
        tutorId: t.id,
        id: t.id,
        ...
        instituteName: instObj ? instObj.name : null,
        avgRating,
        ratingCount,
      };
    });
...
tutorRegisterRouter.post('/register', async (req, res) => {
  try {
    ...
    const existingUser = await store.users.getByEmail(normalizedEmail);
    if (existingUser) {

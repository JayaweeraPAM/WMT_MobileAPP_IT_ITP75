...
export function AdminInstitutesPage() {
  ...
  const [tab, setTab] = useState<'institutes' | 'mgrRequests' | 'tutorRequests'>('institutes');
  const [institutes, setInstitutes] = useState<Institute[]>([]);
  const [mgrRequests, setMgrRequests] = useState<ManagerRegistration[]>([]);
  const [tutorRequests, setTutorRequests] = useState<TutorRequestGroup[]>([]);
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [iRes, mgrRes, tRes] = await Promise.all([
        adminBackendApi.getInstitutes(),
        adminBackendApi.getManagerRegistrations(),
        adminBackendApi.getInstituteRequests(),
      ]);
      setInstitutes(iRes.institutes || []);
      setMgrRequests(mgrRes.registrations || []);
      setTutorRequests(tRes.requests || []);
  ...
  const handleApproveTutorRequestGroup = async (req: TutorRequestGroup) => {
    try {
      await adminBackendApi.approveInstituteRequest({ instituteName: req.instituteName });
      toast.success(`Institute "${req.instituteName}" created successfully!`);
      load();
    } catch (e: any) { toast.error(e.message || 'Failed to create institute'); }
  };

  const handleRejectTutorRequestGroup = async (req: TutorRequestGroup) => {
    try {
      if (!confirm(`Reject all tutor requests for "${req.instituteName}"?`)) return;
      for (const id of req.requestIds) {
        await adminBackendApi.rejectInstituteRequest(id);
      }
      toast.success('Requests rejected');
      load();
    } catch { toast.error('Failed to reject requests'); }
  };
  ...
  return (
    <div className="min-h-screen bg-[#06040f] text-white p-6 md:p-10">
      <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-white/40 text-sm">
                {institutes.length} institutes · {mgrRequests.length} manager requests · {tutorRequests.length} tutor request groups
              </p>
            </div>
          </div>
        </div>
        ...
        {/* Tabs */}
        <div className="flex gap-1 mb-8 p-1 rounded-2xl border border-white/8 w-fit flex-wrap"
          style={{ background: 'rgba(255,255,255,0.03)' }}>
          {([
            { id: 'institutes',     label: 'Institutes',     count: institutes.length },
            { id: 'mgrRequests',    label: 'Manager Requests', count: mgrRequests.length },
            { id: 'tutorRequests',  label: 'Tutor Requests',   count: tutorRequests.length },
          ] as const).map(t => (
        ...
        {tab === 'tutorRequests' && (
          <div className="space-y-4">
            {tutorRequests.length === 0 ? (
              <div className="rounded-3xl border border-white/8 p-16 text-center" style={{ background: 'rgba(255,255,255,0.02)' }}>
                <Users className="w-14 h-14 text-white/20 mx-auto mb-4" />
                <p className="text-white/40 text-lg font-semibold">No tutor requests yet</p>
                <p className="text-white/25 text-sm mt-1">When tutors suggest new institute names, their requests will appear here.</p>
              </div>
                ...
                <div className="flex items-start gap-4 flex-1">
                  ...
                  <div>
                    ...
                    <div className="text-sm mt-2">
                      <span className="text-white/40">Suggested by: </span>
                      <span className="text-[#38BDF8] font-semibold">{req.tutorCount} {req.tutorCount === 1 ? 'Tutor' : 'Tutors'}</span>
                    </div>
                  </div>
                </div>
                <div className="flex gap-2 w-full md:w-auto mt-4 md:mt-0">
                  <button onClick={() => handleRejectTutorRequestGroup(req)}
                    className="flex-1 md:flex-none px-4 py-2.5 rounded-xl font-semibold text-sm text-red-400 border border-red-500/25 bg-red-500/5 hover:bg-red-500/15 transition-all flex items-center justify-center gap-2">
                    <XCircle className="w-4 h-4" /> Reject Group
                  </button>
                  <button onClick={() => handleApproveTutorRequestGroup(req)}
                    className="flex-1 md:flex-none px-4 py-2.5 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 transition-all hover:-translate-y-0.5"
                    style={{ background: 'linear-gradient(135deg,#0EA5E9,#0284C7)', color: 'white', boxShadow: '0 4px 16px rgba(14,165,233,0.3)' }}>
                    <CheckCircle className="w-4 h-4" /> Create Institute
                  </button>
                </div>
              </div>
            ))}

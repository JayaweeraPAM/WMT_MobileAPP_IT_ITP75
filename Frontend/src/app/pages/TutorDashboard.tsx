...
export function TutorDashboard() {
  ...
  return (
    <div className="min-h-screen page-enter" style={{ background: isDark ? 'linear-gradient(135deg, #050814 0%, #0B1120 50%, #111827 100%)' : 'linear-gradient(135deg, #F0F9FF 0%, #FAFCFF 50%, #FAF5FF 100%)', position: 'relative', overflow: 'hidden', minWidth: '100vw', minHeight: '100vh', margin: 0 }}>

      {/* Floating Blobs and Background Bubbles */}
      <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', overflow: 'hidden', zIndex: 0 }}>
        <div style={{ position: 'absolute', width: 600, height: 600, borderRadius: '50%', background: `radial-gradient(circle, rgba(14,165,233,${isDark ? '0.32' : '0.22'}) 0%, transparent 70%)`, filter: 'blur(80px)', top: '-5%', right: '-10%' }} />
        <div style={{ position: 'absolute', width: 500, height: 500, borderRadius: '50%', background: `radial-gradient(circle, rgba(99,102,241,${isDark ? '0.28' : '0.18'}) 0%, transparent 70%)`, filter: 'blur(75px)', bottom: '15%', left: '-5%' }} />
        <div style={{ position: 'absolute', width: 350, height: 350, borderRadius: '50%', background: `radial-gradient(circle, rgba(236,72,153,${isDark ? '0.24' : '0.12'}) 0%, transparent 70%)`, filter: 'blur(60px)', top: '45%', right: '15%' }} />
      </div>
      <style>{`
